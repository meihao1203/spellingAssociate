...
namespace meihao
{
	static myLog* EpollPollerlog  = myLog::getInstance();
	...
	void EpollPoller::waitEpollfd()
	{
		...
		do
		{
			ret = ::epoll_wait( _epollfd,&(*_eventList.begin()),_eventList.size(),10000 );  //等待10s超时
		...
		if(-1==ret)
		{
			...
			EpollPollerlog->info(catMsg("epoll timeout"));
		else 
		{
	...
	void EpollPoller::handleMessage(int fd)
	{
		...
		else
		{
			...
			EpollPollerlog->info(catMsg("no this fd"));
		}
	}
