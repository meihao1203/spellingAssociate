...
namespace meihao
{
	static meihao:: myLog* Threadpoollog  = meihao::myLog::getInstance();
	...
	{
		//cout<<"------更新缓冲开始------"<<endl;
		Threadpoollog->info(catMsg("------更新缓冲开始------"));
		for( auto& elem:_threadsVec )
		...
		//cout<<"------更新缓冲结束------"<<endl;
		Threadpoollog->info(catMsg("------更新缓冲结束------"));
		//写回磁盘
		_cache.writeToFile( (_conf.getMap())["cache"] );
		//cout<<"------cache写回磁盘结束------"<<endl;
		Threadpoollog->info(catMsg("------cache写回磁盘结束------")); 
	}
};
